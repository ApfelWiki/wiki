...
###
### Backup PMWIki pages
### Made by: Stefano Aglietti  [steve@40annibuttati.it]
### http://www.40nnibuttati.it
###
### To use this action you have to insert the following line
...
if ($action=='backup') {
    ...
    # inside the wiki.d directory. The directory should exixt under
    # the PMWiki directory.

    SDV($BackupDir,'/archiv/');

    # Define the format of the compressed file
    # possible value are: .tar, .tgz (GZIP), tbz (BZIP) and .zip
    # Ex. $BackupFormat = ".zip"

    SDV($BackupFormat,".zip");

    # Define the date format
    # 1 = US date MM-DD-YYYY (es. 05-31-2005)
    # 2 = EU date DD-MM-YYYY (es. 31-05-2005)
...
###########################################################
    ...
    if ($BackupFormat != ".tar" && $BackupFormat != ".tgz" && $BackupFormat != ".tbz" && $BackupFormat != ".zip") {
       die( $BackupFormat." is not a correct extension for compressed backup file");
    }

    # Define the date format in the name of backup archive
    If ($DateFormat = 1 ) {
        SDV($BackupFile, $WikiTitle."_".date ("m-d-Y"));
    } else {
        SDV($BackupFile, $WikiTitle."_".date ("d-m-Y"));
    }

$temp = new zip_file($BackupDirPath.$BackupFile.$BackupFormat);
$temp->set_options(array('basedir'=>"$WorkDir",'overwrite'=>1,'level'=>1,'storepaths'=>0));


    $d = dir("$WorkDir");
    SDV($c,'0');
    while ($bakfile = $d->read()) {
        if ($bakfile != "." && $bakfile != ".." && $bakfile != ".flock" && $bakfile != ".lastmod" && $bakfile != ".htaccess") {
            $temp->add_files($bakfile);
            echo $c." - ".$bakfile." done<br>\n";
            $c = $c + 1;
        }
    }
    $d->close();
$temp->create_archive();
    ...
    If ($ShowLink) {
        echo "<br /><br />You can download the backup file now: <a href='".$ScriptUrl.$BackupDir.$BackupFile.$BackupFormat."'>".$ScriptUrl.$BackupDir.$BackupFile.$BackupFormat."</a>";
    } else {
